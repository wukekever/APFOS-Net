...
def load_param(net, path):
    if os.path.exists(path):
        ...
        Xavier_initi(net)
        save_param(net, path="./pkl_uniform_case_3/Xavier_initi_net_params_case_3.pkl")


# In[ ]:


load_param(net, path="./pkl_uniform_case_3/Xavier_initi_net_params_case_3.pkl")
...
Iter = 100000
theta, m, w, eps = 0, 1, 2, 10 ** (-20)
xz = generate_sample(data_size_temp=200)
xz_left = generate_sample_bdy_left(boundary_data_size_temp=200)
xz_right = generate_sample_bdy_right(boundary_data_size_temp=200)
xz_down = generate_sample_bdy_down(boundary_data_size_temp=200)
xz_up = generate_sample_bdy_up(boundary_data_size_temp=200)
xz.requires_grad = True
...
for it in range(Iter):
    ...
    if it % 1 == 0:
        ...
        print("")
    path = "./pkl_uniform_case_3/net_params_case_3_{}.pkl".format(it)
    if it % 1000 == 0:
        save_param(net, path)
...
save_param(net, path="./pkl_uniform_case_3/net_params_case_3.pkl")
np.save("loss_uniform_case_3.npy", loss_record)
np.save("error_uniform_case_3.npy", error_record)
time_end = time.time()
